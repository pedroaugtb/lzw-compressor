...
def plot_execution_time_decompression(stats_subset):
    data = []
    for stats in stats_subset:
        if 'execution_time_seconds' in stats and stats['operation'] == 'Descompressão':
            data.append({
                'Tipo de Arquivo': stats.get('file_type'),
                'Tempo de Descompressão (s)': stats['execution_time_seconds']
            })
    if data:
        df = pd.DataFrame(data)
        fig = go.Figure(data=[
            go.Bar(x=df['Tipo de Arquivo'], y=df['Tempo de Descompressão (s)'], name='Descompressão')
        ])
        fig.update_layout(
            title='Tempo de Execução da Descompressão',
            yaxis_title='Tempo (segundos)',
            template='plotly_dark'
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.warning("Nenhum dado disponível para gerar o gráfico de Tempo de Execução da Descompressão.")
...
# Seção: Tempo de Execução da Descompressão
st.subheader("Tempo de Execução da Descompressão")
st.markdown("""
Além da compressão, o tempo necessário para descomprimir os arquivos também é analisado. Isso é essencial para entender a eficiência total do algoritmo.
""")
plot_execution_time_decompression(all_stats)
